...
- (void)checkPassword:(NSString*)string
{
    ...
    // 验证密码错误
        ...
        if (nRetryTimesRemain > 0) {
            ...
            [user removeObjectForKey:@"userpic"];
            [user removeObjectForKey:@"username"];
            [user removeObjectForKey:@"usermoney"];
            [user removeObjectForKey:@"gender"];
            [user removeObjectForKey:@"birthday"];
            [user removeObjectForKey:@"headImage"];
            [user removeObjectForKey:@"phone"];
            [self showAlert:@"超过最大次数，请重新登录！"];
        }
