...
//        var x : CGFloat = 200,y : CGFloat = 100
        ...
        y += kTextHeight + 14.0
        for txt in inputs {
            txt.frame = CGRect(x:12, y:y-100, width:kWindowWidth - 24, height:30)
            txt.layer.cornerRadius = 3
            y += 40
        }
        // Buttons 添加按钮
//        for btn in buttons {
//            btn.frame = CGRect(x:12, y:y, width:60, height:35)
//            btn.layer.cornerRadius = 3
//            y += 45.0
////            x += 70.0
//            
//            println("btn-->\(btn.currentTitle),x-->\(x),y-->\(y)")
//        }
        
        x = 12
        y -= 70
        for btn in buttons {
            btn.frame = CGRect(x:x, y:y, width:60, height:35)
            btn.layer.cornerRadius = 3
